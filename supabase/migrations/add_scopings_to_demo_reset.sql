...
-- as further extended by add_phases_to_demo_reset.sql and
-- add_issue_logs_to_demo_reset.sql) to cover the new scopings table
-- (scopings.sql). Without this, a demo-project visitor's Scoping Q&A would
-- persist forever instead of resetting nightly like every other document
-- type. Run this AFTER scopings.sql.
--
-- Same shadow-table approach as charters/risk_logs, which scopings is
-- structurally identical to (project_id FK, one row per project via unique
-- index, jsonb column) - `like scopings including defaults`, no FK
-- dependents, so it slots in next to charters everywhere below with no
-- special ordering concerns.

create table if not exists scopings_demo_snapshot (like scopings including defaults);

alter table scopings_demo_snapshot enable row level security;
...
-- Bake in a fresh baseline that includes scopings (empty on every demo
-- project until this runs, same as issue_logs was before its own migration).
select public.capture_demo_snapshot();

-- Verify afterward:
--
--   select (select count(*) from scopings_demo_snapshot) as scopings;
--
