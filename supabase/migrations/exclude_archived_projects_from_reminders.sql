-- Excludes archived projects (projects.status = 'Archived') from
-- send_task_due_reminders(), alongside the existing demo-project exclusion
-- (exclude_demo_projects_from_reminders.sql). Archiving a project doesn't
-- touch its tasks' due_date or status, so without this, an owner/assignee
-- kept getting due/overdue reminders for tasks on a project everyone
-- considers closed.
--
-- projects.status is Title Case ('Active' / 'Archived') - not the same
-- casing as tasks.status (lowercase, e.g. 'not_started'). Matched exactly
-- as 'Archived' here, per how ProjectDetailLayout.jsx's toggleArchived()
-- writes it.
--
-- Only change: "and p.status <> 'Archived'" added to both Rule A and Rule B
-- WHERE clauses, alongside the existing p.is_demo = false condition.
...
--   where project_id in (select id from projects where status = 'Archived')
