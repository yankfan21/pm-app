-- Fixes a repeat 429 from Resend on retest of send_task_due_reminders()
-- (task_notification_cron_timeout_fix.sql), even with pg_sleep(0.5) already
-- in place. 0.5s between calls = exactly 2 requests/second, right at
-- Resend's rate limit with zero safety margin - any small timing jitter
-- (network latency, pg_net scheduling) tips one call over it.
--
-- Fix: pg_sleep(0.5) -> pg_sleep(1) after both net.http_post calls (assignee
-- send, owner send). Real headroom under the limit instead of running
-- exactly at it. Costs more total runtime, still fine for a once-daily batch
-- job (a project's worth of due/overdue tasks, not a mass mailer).
--
-- Everything else unchanged from task_notification_cron_timeout_fix.sql -
-- same row selection (Rule A / Rule B by row shape), same reminder-day
-- logic, same recipients/dedup, same timeout_milliseconds := 10000 on both
-- net.http_post calls. See task_notification_cron.sql's header comments for
-- the full original rationale; not repeated here.
--
-- create or replace function, so this is safe to run regardless of whether
-- task_notification_cron_timeout_fix.sql has fired yet - it just replaces
  ...
  loop
    ...
    if rec.assignee_user_id is not null then
      ...
      if v_assignee_email is null then
        ...
        -- Resend rate-limits per second - 0.5s was exactly 2 req/s, right at
        -- the limit with no margin. 1s gives real headroom. Once-daily batch
        -- job, so the extra runtime is a non-issue.
        perform pg_sleep(1);
      end if;
...
-- (select public.send_task_due_reminders();) and confirm no 429s in
