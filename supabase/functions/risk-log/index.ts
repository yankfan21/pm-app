...
// Bumped from 1500 - "generate"/"suggest" return an open-ended list of risk
// rows (no upper bound like task-gen/backlog-gen's "4 to 12"), the same
// class of ceiling that truncated charter's apply_followup in production
// (Claude's "thinking" content block draws from the same max_tokens budget
// as the actual JSON output). max_tokens is only a ceiling, so raising it
