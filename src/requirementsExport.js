...
export function exportRequirementsBriefPdf(project, values) {
  ...
  ensureSpace(18)
  doc.text('REQUIREMENTS / DISCOVERY BRIEF', marginX, y)
  y += 30
  doc.setTextColor(0)
  ...
  doc.save(`${sanitizeFilename(project.name)}-Requirements-Brief.pdf`)
}

export async function exportRequirementsBriefDocx(project, values) {
  const children = [
    new Paragraph({ text: project.name, heading: HeadingLevel.TITLE }),
    new Paragraph({ text: 'Requirements / Discovery Brief', spacing: { after: 300 } }),
  ]
  ...
  downloadBlob(blob, `${sanitizeFilename(project.name)}-Requirements-Brief.docx`)
}
