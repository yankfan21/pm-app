...
export function exportCommsPdf(project, values, variant) {
  const { title, sections } = COMMS_VARIANTS[variant]
  ...
  doc.save(`${sanitizeFilename(project.name)}-${sanitizeFilename(title)}.pdf`)
}

export async function exportCommsDocx(project, values, variant) {
  const { title, sections } = COMMS_VARIANTS[variant]
  const children = [
  ...
  downloadBlob(blob, `${sanitizeFilename(project.name)}-${sanitizeFilename(title)}.docx`)
}
